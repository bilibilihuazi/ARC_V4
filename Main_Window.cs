...
namespace ARC_V4
{
    public partial class Main_Window: AntdUI.Window
    {
        ...
        #endregion
        //变量========================================================================================
        public static string Title = "自动点名器V4";//标题
        public static string Fork = "Indev";//分支
        public static string Version = "4.0.0.0";//版本
        public static string RunPath = Directory.GetCurrentDirectory();//运行路径
        public static string ConfigPath = $"{RunPath}\\config\\config.ini";//配置文件路径
        ...
        public Main_Window()
        {
            ...
            {
                WriteConfig(ConfigPath, "config", "Mode", "0");
            ...
            {
                WriteConfig(NamelistPath, "info", "num", "12");
                WriteConfig(NamelistPath, "names", "1", "小明");
                WriteConfig(NamelistPath, "names", "2", "小红");
                WriteConfig(NamelistPath, "names", "3", "小军");
                WriteConfig(NamelistPath, "names", "4", "明明");
                WriteConfig(NamelistPath, "names", "5", "红红");
                WriteConfig(NamelistPath, "names", "6", "卡卡");
                WriteConfig(NamelistPath, "names", "7", "妮妮");
                WriteConfig(NamelistPath, "names", "8", "科比·布莱恩特");
                WriteConfig(NamelistPath, "names", "9", "华某人");
                WriteConfig(NamelistPath, "names", "10", "戈登·弗里曼");
            ...
            try
            {
                tabs.SelectedIndex = int.Parse(ReadConfig(ConfigPath, "config", "Mode"));
            }
            catch(Exception ex)
            {
                ...
                {
                    Title = "发生错误！",
                    Text = $"在读取配置 config -> Mode 时发生错误，其值只能为整形。而错误的出现了 {ReadConfig(ConfigPath, "config", "Mode")} ！\n返回值:( {ex.Message} )",
            ...
            try
            {
                labe_MoDisplay.Text = ReadConfig(ConfigPath, "config", "WelcomeText");
                label_AutoDisplay.Text = ReadConfig(ConfigPath, "config", "WelcomeText");
            }
            catch (Exception ex)
            {
                AntdUI.Notification.open(new AntdUI.Notification.Config(this, "", "", AntdUI.TType.None, AntdUI.TAlignFrom.TR)
                {
                    Title = "发生错误！",
                    Text = $"在读取配置 config -> WelcomeText 时发生错误\n返回值:( {ex.Message} )",
                    Icon = AntdUI.TType.Error
                });
            }

            //读取设置配置
            inputNumber_Settings_Time.Value = int.Parse(ReadConfig(ConfigPath, "config", "Time"));
            timer_Auto.Interval = int.Parse(ReadConfig(ConfigPath, "config", "Time"));
            timer_Mo.Interval = int.Parse(ReadConfig(ConfigPath, "config", "Time"));

            inputNumber_Settings_AutoTime.Value = int.Parse(ReadConfig(ConfigPath, "config", "AutoTime"));
        ...
        {
            if (button_MoStart.Type == AntdUI.TTypeMini.Primary)
            {
                button_MoStart.Text = "停止抽取";
                button_MoStart.Type = AntdUI.TTypeMini.Error;
                button_MoStart.Icon = Properties.Resources.Pause;
                MoState = true;
            }
            else
            {
                button_MoStart.Text = "开始抽取";
                button_MoStart.Type = AntdUI.TTypeMini.Primary;
                button_MoStart.Icon = Properties.Resources.Start;
                MoState = false;

                AntdUI.Notification.open(new AntdUI.Notification.Config(this, "", "", AntdUI.TType.None, AntdUI.TAlignFrom.TR)
                {
                    Title = "恭喜！",
                    Text = $"恭喜您抽到了 {labe_MoDisplay.Text}",
        ...
        private void timer_Mo_Tick(object sender, EventArgs e)
        {
            if (MoState)
            {
                labe_MoDisplay.Text = ReadConfig(NamelistPath, "names", $"{random.Next(1, int.Parse(ReadConfig(NamelistPath, "info", "num")))}");
            }
        }

        private async void button_AutoStart_Click(object sender, EventArgs e)
        {
            button_AutoStart.Text = "自动抽取中...";
            button_AutoStart.Icon = Properties.Resources.Pause;
            button_AutoStart.Loading = true;
            AutoState = true;

            float waveValue = 0F;
            for (int i = 0; i < 100; i++)
            {
                button_AutoStart.LoadingWaveValue = waveValue;
                waveValue = waveValue + 0.01F;
                await Task.Delay(int.Parse(ReadConfig(ConfigPath, "config", "AutoTime")) / 100);
            }

            button_AutoStart.Text = "开始抽取";
            button_AutoStart.Icon = Properties.Resources.Start;
            button_AutoStart.Loading = false;
            AutoState = false;

            AntdUI.Notification.open(new AntdUI.Notification.Config(this, "", "", AntdUI.TType.None, AntdUI.TAlignFrom.TR)
            {
                Title = "恭喜！",
                Text = $"恭喜您抽到了 {label_AutoDisplay.Text}",
                Icon = AntdUI.TType.Success
            });
        }
        ...
        private void timer_Auto_Tick(object sender, EventArgs e)
        {
            if (AutoState)
            {
                label_AutoDisplay.Text = ReadConfig(NamelistPath, "names", $"{random.Next(1, int.Parse(ReadConfig(NamelistPath, "info", "num")))}");
            }
        }

        private void inputNumber_Settings_Time_ValueChanged(object sender, AntdUI.DecimalEventArgs e)
        {
            WriteConfig(ConfigPath, "config", "Time", $"{inputNumber_Settings_Time.Value}");
            timer_Auto.Interval = int.Parse(ReadConfig(ConfigPath, "config", "Time"));
            timer_Mo.Interval = int.Parse(ReadConfig(ConfigPath, "config", "Time"));
        }
